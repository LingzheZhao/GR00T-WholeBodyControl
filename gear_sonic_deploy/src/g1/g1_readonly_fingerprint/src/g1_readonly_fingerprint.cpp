...
g1_fingerprint::Snapshot MakeSnapshot(const LowState& state) {
  ...
  for (std::size_t index = 0; index < snapshot.q.size(); ++index) {
    snapshot.q[index] = state.motor_state()[index].q();
    snapshot.dq[index] = state.motor_state()[index].dq();
  }
  return snapshot;
}
...
std::string ErrorJson(const std::string& error) {
  return std::string{
      ...
      + ",\"diagnostic_findings\":[\"runtime_error\"],\"error\":"
